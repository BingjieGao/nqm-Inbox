...
function send() {
  webix.message(JSON.stringify($$("mailform").getValues(), null, 2));
  // webix.ajax('')
}
var ui = { rows:[
  {
...
webix.ready(function() {
  ...
  $$("$datatable1").bind($$("$tree1"),function(obj,filter){
    console.log('datatable obj is => '+ JSON.stringify(obj));
    return obj.folder == filter.id;
  });

  $$("$datatable1").attachEvent("onAfterSelect",function(id){
    console.log('datatble selected');
    console.log('selected id is => + '+this.getItem(id).id);
  ...
  $$('id_delete').attachEvent('onItemClick',function(id,e){
    console.log("delete email id => "+$$("$datatable1").getSelectedId().id);
