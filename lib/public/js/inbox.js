...
function send() {
  ...
  webix.ajax().post("/send",{message:new_message,content:new_content},function(text,data,xmlHttpRequest){
    ...
    if(xmlHttpRequest.readyState == 4 && xmlHttpRequest.status == 200){
      $$('popupwin').close();
      webix.message('sent success',null,10);
    }
  })
}
var ui = { rows:[
    ...
    rows:[
      {
        ...
        {view:"button", type:"icon", icon:"cog", gravity:1,width:120,label:"Settings"}
      ]
      },
      {
        type:"wide", cols:[
...
webix.ready(function() {
  ...
  $$('id_delete').attachEvent('onItemClick',function(id,e){
    ...
    webix.ajax().del("/message?id="+gData[$$('$datatable1').getSelectedId().id-1].id,function(text, data, XmlHttpRequest){
